...
const createPost =async(req,res)=>{
    try {
        ...
        if(!postedBy || !text)
        {
            return req.status(400).json({error: "PostedBY and text field are empty"});
        }
        ...
        {
            return res.status(401).json({error:"Uauthorised to create Post "});
        }
...
const deletePost = async(req,res)=>{
    try {
        ...
        if(post.postedBy.toString()!== req.user._id.toString()){
            return res.status(401).json({error: "Unauthorised to delete this post"});
        }
